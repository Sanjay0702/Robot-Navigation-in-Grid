...
class Terrain:
    ...
    def create_robot(self, robot_id):
        if robot_id in self.robots:
            ...
            print(f"Robot with ID {robot_id} is created successfully!!!.")
    ...
    def move_robot(self, robot_id, direction_and_steps):
        if robot_id not in self.robots:
            ...
            if len(direction_and_steps) != 2 or direction_and_steps[0].upper() not in ["N","S","E","W"] and not direction_and_steps[1].isdigit():
                print("Invalid Input Format for Direction and Steps")
            else:
                ...
                print(robot.move(direction, steps, occupied_positions))

    def get_robot_position(self, robot_id):
        if robot_id not in self.robots:
            print(f"Robot with ID {robot_id} does not exist.")
        else:
            print(f"The position of {robot_id} is {self.robots[robot_id].position}.")
